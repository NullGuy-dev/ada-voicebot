...
AAS_NOPE = "smartSpeak_NOPE"

settingsFile = funcs.getDataFromJson("setting.json")
botQueryData = funcs.getDataFromJson("bot_data.json")
botSYSData = funcs.getDataFromJson("bot_sys_data.json")

now = datetime.datetime.now()
month = botSYSData["monthClasses"][now.month-1]

def startBot():
    for text in funcs.listen(stream,rec):
        funcs.mainDB_BOT(command=text.lower(), dataForSAYYN=[AAS_YEAH, AAS_NOPE],
                         settingsDataFile=settingsFile,
                         botQuetionDataJson=botQueryData,
                         botSYSDataFile=botSYSData,
                         nowVar=now,
                         monthVar=month)
startBot()

# startBotT = Thread(target=startBot, args=())
# startBotT.start()
