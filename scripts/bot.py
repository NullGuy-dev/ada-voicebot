...
settingsFile = fn.getDataFromJson("setting.json")
botSYSData = fn.getDataFromJson("bot_sys_data.json")

now = datetime.datetime.now()
month = botSYSData["monthClasses"][now.month-1]

loopCheck = fn.isConnected()
botSYSData["IsConnected"] = loopCheck
fn.setDataToJson("bot_sys_data.json", botSYSData)

# startBotT = Thread(target=fn.startBot, args=(stream, rec, now, month, botSYSData))
# loopCheckT = Thread(target=fn.threadCheckIsConnected, args=())
# loopCheckT.start()
# startBotT.start()
#
while True:
